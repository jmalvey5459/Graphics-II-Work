...
void a3demo_loadFramebuffers(a3_DemoState *demoState)
{
	...
	a3framebufferCreate(fbo, 1, a3fbo_colorRGBA8, a3fbo_depth24_stencil8, demoState->frameWidth, demoState->frameHeight);
}
...
//	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	...
	currentDrawable = demoState->draw_sphere;
	a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uMVP, 1, demoState->camera->viewProjectionMat.mm);

	// ****TO-DO: 
	// 1. depth buffer hax: 
	//	- see what happens with depth test disabled for all shapes
	//	- change color of first sphere to fully transparent, or use color mask
	//	- play with other depth buffer settings
...
/*
	...
	*/
	...
	switch (demoState->demoMode)
	{
		// display previously-rendered scene as a texture on shapes
	case 1:
		// enable depth test and clear
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		glEnable(GL_DEPTH_TEST);

		// render teapot or something interesting
		// use lighting and texturing program for maximum awesomeness
	...
	if (demoState->textInit && demoState->showText)
	{
		...
		const char *demoModeText[] = {
			"Shapes rendered in scene (buffering tests)",
			"Scene rendered on shapes (what)",
			"Scene rendered on FSQ",
		};

		const float col = (!demoState->displayDepth || demoState->demoMode < 2) ? 1.0f : 0.0f;
		...
		if (demoState->demoMode > 0)
			a3textDraw(demoState->text, -0.98f, +0.70f, -1.0f, col, col, col, 1.0f,
				"Showing %s buffer as texture (toggle = 'f')", demoState->displayDepth ? "depth" : "color");
